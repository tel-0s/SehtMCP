...
public sealed class DistributionTools(PluginWorkspace workspace, AssetService assets, SehtConfig config)
{
    ...
    {
        format = "seht-manifest-1", generator = "SehtMCP 0.3.0", plugin = s.Mod.ModKey.ToString(), revision = s.Revision, dirty = s.Dirty, isMaster = s.Mod.IsMaster, isLight = s.Mod.IsSmallMaster, masters = s.Masters.Select(m => m.ToString()).ToArray(), sha256 = s.SavedHash,
