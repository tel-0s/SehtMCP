...
public sealed class AuthoringTools(PluginWorkspace workspace, SehtConfig config, ExternalTools external)
{
    ...
    public CallToolResult Status() => Run(() => new { version = "0.2.0", workspace = config.Workspace, dataRoots = config.DataRoots, external = external.Status(), sessions = workspace.List(), runtime = Environment.Version.ToString(), transport = "stdio", capabilities = new[] { "typed-plugin-authoring", "esp-esm-esl-espfe", "transactional-batches", "checkpoints", "structural-validation", "bsa-read", "nif-inspect", "nif-png-preview", "papyrus-compile", "interior-navmesh-generation", "recast-cell-render-geometry", "navmesh-door-links", "navmesh-png-preview" } });
